...
// Step-by-step diagnostics for the motion pipeline — surfaces which stage
// fails (token vs amp-api vs m3u8) when running in a remote/serverless env.
export async function diagnoseMotion({ id, kind } = {}) {
  const steps = {};
  // 1. dev token
  let token;
  try {
    ...
    const bundle = html.match(/\/assets\/index[^"']*\.js/)?.[0];
    steps.browseHtml = { ok: true, len: html.length, bundle: bundle || null };
    if (!bundle) throw new Error('bundle not found in browse HTML');
    const js = await fetchText(`https://music.apple.com${bundle}`, { timeout: 15000 });
    const jwt = js.match(/eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/)?.[0];
    steps.bundleJs = { ok: true, len: js.length, jwtFound: !!jwt };
    if (!jwt) throw new Error('jwt not found in bundle');
    token = jwt;
    steps.token = { ok: true, preview: jwt.slice(0, 24) + '…' };
  } catch (e) {
    steps.token = { ok: false, error: `${e.name}: ${e.message}` };
    return { ok: false, steps };
  }

  // 2. amp-api
  const type = TYPE_MAP[kind];
  const ampUrl = `https://amp-api.music.apple.com/v1/catalog/us/${type}/${id}?extend=editorialVideo`;
  let masters = [];
  try {
    const t = withTimeout(TIMEOUT_MS);
    let r;
    try {
      r = await fetch(ampUrl, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Origin': 'https://music.apple.com',
          'Accept': 'application/json',
    ...
    steps.ampApi = { ok: r.ok, status: r.status, len: body.length };
    if (!r.ok) {
      steps.ampApi.bodyPreview = body.slice(0, 300);
      return { ok: false, steps };
    }
    const data = JSON.parse(body);
    const ev = data?.data?.[0]?.attributes?.editorialVideo || {};
    masters = [...new Set(Object.values(ev).map((v) => v?.video).filter((v) => typeof v === 'string'))];
    steps.editorialVideo = { keys: Object.keys(ev), masters };
  } catch (e) {
    steps.ampApi = { ok: false, error: `${e.name}: ${e.message}` };
    return { ok: false, steps };
  }
  ...
  steps.variants = { squareKeys: Object.keys(square), tallKeys: Object.keys(tall) };

  return { ok: true, steps, best: pickBest(square), bestTall: pickBest(tall) };
}
